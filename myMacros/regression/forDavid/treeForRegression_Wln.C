...
void treeForRegression_Wln::Loop()
{
  if (fChain == 0) return;
   ...
   //TFile *outfile=new TFile("/exports/uftrig01a/dcurry/data/bbar/13TeV/heppy/v14/trees_for_regression/ZH125_ZH_genJet_weighted.root", "recreate");
   
   TFile *outfile=new TFile("/exports/uftrig01a/dcurry/data/bbar/13TeV/heppy/v14/trees_for_regression_hJets/ZH125_genJet.root", "recreate");

   int Jet_vtxNtrk;
   float Jet_corr_,Jet_mass_,Jet_rawPt_, Jet_e_, Jet_mt_, rho_, Jet_pt_,Jet_eta_,Jet_phi_, Jet_leptonPtRel_,Jet_leptonDeltaR_,Jet_leptonPt_, Jet_leptonPtRelInv_, Jet_leadTrackPt_, Jet_chHEF_, Jet_chEmEF_, Jet_neHEF_, Jet_neEmEF_, Jet_vtx3dL_, Jet_vtx3deL_, Jet_chMult_, Jet_mcPt_, Jet_mcEta_, Jet_mcPhi_, Jet_mcM_,  Jet_vtxMass_, Jet_csv_, Jet_flavor_,  met_pt_, met_phi_, Jet_btagCMVA_, Jet_mcFlavour_, Jet_id_, Jet_puId_, Jet_met_dPhi_, Jet_mcE_, Jet_btagCSV_, Jet_mcMatchId_, Jet_mt_, Jet_vtxMass_, Jet_vtx3dL, Jet_vtx3deL, Jet_vtxPt_, dR_;
   Jet_mass_=-999,Jet_rawPt_=-999, Jet_e_=-999, Jet_mt_=-999, rho_=-999, Jet_pt_=-999,Jet_eta_=-999,Jet_phi_=-999, Jet_leptonPtRel_=-999,Jet_leptonDeltaR_=-999,Jet_leptonPt_=-999, Jet_leptonPtRelInv_=-999, Jet_leadTrackPt_=-999, Jet_chHEF_=-999, Jet_chEmEF_=-999, Jet_neHEF_=-999, Jet_neEmEF_=-999, Jet_vtx3dL_=-999, Jet_vtx3deL_=-999, Jet_chMult_=-999, Jet_mcPt_=-999, Jet_mcEta_=-999, Jet_mcPhi_=-999, Jet_mcM_=-999,  Jet_vtxMass_=-999, Jet_csv_=-999, Jet_flavor_=-999,  met_pt_=-999, met_phi_=-999, Jet_btagCMVA_=-999, Jet_mcFlavour_=-999,  Jet_id_=-999, Jet_puId_=-999 , Jet_met_dPhi_=-999, Jet_mcE_=-999, Jet_btagCSV_=-999, Jet_mcMatchId_=-999, Jet_mt_=-999, Jet_vtxMass_=-999., Jet_vtx3dL=-999., Jet_vtx3deL=-999., Jet_vtxPt_ =-999, Jet_vtxNtrk=-999, Jet_corr_=-999;
...
outtree->Branch("Jet_met_dPhi", &Jet_met_dPhi_, "Jet_met_dPhi_/F");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (jentry%10000==0)
	std::cout << "-----> Event # " << jentry << endl;


      // if(!(Vtype==0  || Vtype==1)) continue;
      // if(Vtype!=4) continue;	
      // if(!(Vtype==2  || Vtype==3)) continue;
      
      
      // Loop over all jets
      //for(int i =0 ; i<nJet; i++){
      
      // Loop over higgs jets
      for (int i = 0; i < 2; i++) {
	
	
	TLorentzVector hJ0, hJ1;
        Jet_pt_=Jet_pt[hJCidx[i]];
        Jet_eta_=Jet_eta[hJCidx[i]];
        Jet_phi_=Jet_phi[hJCidx[i]];
        Jet_rawPt_=Jet_rawPt[hJCidx[i]];
	Jet_corr_=Jet_corr[hJCidx[i]];
	Jet_puId_ = Jet_puId[hJCidx[i]];
        Jet_mass_=Jet_mass[hJCidx[i]];
        Jet_met_dPhi_=deltaPhi(met_phi,Jet_phi[hJCidx[i]]);
        met_pt_ = met_pt;
        met_phi_= met_phi;
 	Jet_leptonPtRel_ = TMath::Max(0.,Jet_leptonPtRel[hJCidx[i]]);
	Jet_leptonDeltaR_ = TMath::Max(0.,Jet_leptonDeltaR[hJCidx[i]]);
	Jet_leptonPt_ = TMath::Max(0.,Jet_leptonPt[i]);
	Jet_leptonPtRelInv_ = TMath::Max(0.,Jet_leptonPtRelInv[hJCidx[i]]);
	Jet_leadTrackPt_ = Jet_leadTrackPt[hJCidx[i]];
	Jet_chHEF_ = Jet_chHEF[hJCidx[i]];
	Jet_chEmEF_=  Jet_chEmEF[hJCidx[i]];
	Jet_neHEF_= Jet_neHEF[hJCidx[i]];
	Jet_neEmEF_ = Jet_neEmEF[hJCidx[i]];
	Jet_btagCSV_ = Jet_btagCSV[hJCidx[i]];

        hJ0.SetPtEtaPhiM(Jet_pt[hJCidx[i]],Jet_eta[hJCidx[i]],Jet_phi[hJCidx[i]],Jet_mass[hJCidx[i]]);
        rho_ = rho;
        Jet_e_=hJ0.E();
        Jet_mt_=hJ0.Mt();
        Jet_chMult_= Jet_chMult[hJCidx[i]];
        Jet_vtx3dL_= TMath::Max(0.,Jet_vtx3DVal[hJCidx[i]]);
	Jet_vtxMass_= TMath::Max(0.,Jet_vtxMass[hJCidx[i]]);
        Jet_vtx3deL_= TMath::Max(0.,Jet_vtx3DSig[hJCidx[i]]);
        Jet_vtxNtrk= TMath::Max(0.,Jet_vtxNtracks[hJCidx[i]]);
	Jet_vtxPt_= TMath::Max(0.,Jet_vtxPt[hJCidx[i]]);

	// Use genJet as target
        //if(Jet_mcIdx[i] < 0 ||Jet_mcIdx[i] > 15 ) continue;
	
	//Jet_mcPt_= GenJet_wNuPt[Jet_mcIdx[i]];
	//Jet_mcEta_=Jet_mcEta[i];
	//Jet_mcPhi_=Jet_mcPhi[i];
	//Jet_mcM_=Jet_mcM[i];
	//hJ1.SetPtEtaPhiM(Jet_mcPt[i],Jet_mcEta[i],Jet_mcPhi[i],Jet_mcM[i]);
	//Jet_mcE_=hJ1.E();
		
	// Use bquark as target
	/*
	dR_ =999;
	double minDr = 0.4;
	for(int m=0; m<nGen_Jets; m++){
	...
	*/
	...
	dR_ =999;
        double minDr = 0.4;
        for(int m=0; m<nGenJet; m++){

          hJ1.SetPtEtaPhiM(GenJet_wNuPt[m],GenJet_wNuEta[m], GenJet_wNuPhi[m], GenJet_wNuM[m]);

          if(hJ1.DeltaR(hJ0)<minDr){
            Jet_mcE_=hJ1.E();
            Jet_mcPt_ = GenJet_wNuPt[m];
            Jet_mcEta_= GenJet_wNuEta[m];
            Jet_mcPhi_= GenJet_wNuPhi[m];
            Jet_mcM_  = GenJet_wNuM[m];
            minDr = hJ1.DeltaR(hJ0);
            dR_ = minDr;
          }
        } // end genJet loop
	...
	Jet_mcFlavour_ = Jet_hadronFlavour[i];
