...
prefix = '/exports/uftrig01a/dcurry/data/bbar/13TeV/heppy/files/jetPt_weighted/'
...
fB = 'v14_11_2015_ZH125.root'
infileB = ROOT.TFile(prefix + '/' + fB, 'read')
treeB = infileB.Get('tree') 

#for event in treeB:
nentry = tree.GetEntries()
print 'Looping over %i events in file %s' %(nentry,fB)
for iEvt in range(nentry):
  
  treeB.GetEntry(iEvt)
  
  ptEtaWeight = tree.weight_etaPt
  histos[0].Fill(tree.Jet_pt, tree.weight_etaPt)
  histos[1].Fill(tree.Jet_pt )
  histos[2].Fill(tree.Jet_pt, tree.Jet_eta )
