...
for iter in range(0,1):
    ...
    output = TFile.Open('/exports/uftrig01a/dcurry/data/bbar/13TeV/heppy/regression/jan_zll_genJet_boost/v14_11_2015_ggZH125.root', 'recreate')

    regWeight = 'weights_cat_ZH_genJet/TMVARegression_BDTG.weights.xml'
    ...
    Jet_mcPhiq = array('f',[0]*2)	
    Jet_mcEtaq = array('f',[0]*2)	
    ...
    newtree.Branch('Jet_regPt',Jet_regPt,'Jet_regPt[2]/F')
    newtree.Branch('Jet_mcPtq',Jet_mcPtq,'Jet_mcPtq[2]/F')
    newtree.Branch('Jet_mcEtaq',Jet_mcEtaq,'Jet_mcEtaq[2]/F')
    newtree.Branch('Jet_mcPhiq',Jet_mcPhiq,'Jet_mcPhiq[2]/F')		
    ...
    for entry in range(0,nEntries):
	    ...
	    minDr1 = 0.4;
    		...
    		if hQ.DeltaR(hJ0)<minDr0 :
				...
				Jet_mcEtaq[0] = tree.GenBQuarkFromH_eta[m]
				Jet_mcPhiq[0] = tree.GenBQuarkFromH_phi[m]
              			minDr0 = hQ.DeltaR(hJ0)
		if hQ.DeltaR(hJ1)< minDr1:

                                Jet_mcPtq[1] = tree.GenBQuarkFromH_pt[m]
				Jet_mcEtaq[1] = tree.GenBQuarkFromH_eta[m]
                                Jet_mcPhiq[1] = tree.GenBQuarkFromH_phi[m]
            ...
            #    fathFilterJets_e0 = tree.fathFilterJets_e[0]
                ...
                #if not (value == 'Jet_MET_dPhi' or value == 'METet'  or value == 'Jet_corr' or value == "Jet_et" or value == 'Jet_mt'):
            ...
            #    if not (value == 'Jet_MET_dPhi' or value == 'METet' or value == "rho25" or value == "Jet_et" or value == 'Jet_mt' or value == 'Jet_corr'):
            #        theVars0FJ[key][0] = theFormsFJ["form_reg_%s_0" %(key)].EvalInstance()
            #        theVars1FJ[key][0] = theFormsFJ["form_reg_%s_1" %(key)].EvalInstance()
            #Jet_MET_dPhi[0] = deltaPhi(METphi[0],Jet_phi_0)
            #Jet_MET_dPhi[1] = deltaPhi(METphi[0],Jet_phi_1)
            #Jet_MET_dPhiArray[0][0] = deltaPhi(METphi[0],Jet_phi_0)
            #Jet_MET_dPhiArray[1][0] = deltaPhi(METphi[0],Jet_phi_1)
	    ...
	    for key in regVars:
		    #print key	
                    ...
                    theForms["form_reg_%s_0" %(key)].EvalInstance()
                    theForms["form_reg_%s_1" %(key)].EvalInstance()
		...
		HNoRegwithFSR = ROOT.TLorentzVector()
                ...
                hJ0.SetPtEtaPhiM(rPt0,Jet_eta_0,Jet_phi_0,0.)#tree.Jet_mass[tree.hJCidx[0]])
                hJ1.SetPtEtaPhiM(rPt1,Jet_eta_1,Jet_phi_1,0.)#tree.Jet_mass[tree.hJCidx[1]])
                Jet_regPt[0] = rPt0
                Jet_regPt[1] = rPt1
                H.HiggsFlag = 1
                H.mass = (hJ0+hJ1).M()
