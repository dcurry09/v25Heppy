...
file = TFile('/exports/uftrig01a/dcurry/heppy/v21/DY_merged.root')
...
alep_id_noTrig = TH1F('alep_id_noTrig', '',50 , -1.1, 1.1)

alep_eta = TH1F('alep_eta', '',50 , -2.5, 2.5)

alep_eta_id = TH2F('alep_eta_id', '',50 , -2.5, 2.5, 50, -1.1, 1.1)

hist_list = [alep_pt, alep_iso, alep_id, alep_eta, alep_eta_id, alep_id_noTrig]
# ========================================
...
for iEvt in range(tree.GetEntries()):
    ...
    if tree.VtypeSim != 1: continue 

    if tree.Vtype !=3: continue

    if tree.nvLeptons < 1: continue

    if tree.V_pt < 50: continue

    #print tree.vLeptons_mcMatchId[0]

    if tree.vLeptons_mcMatchId[0] != 23: continue
    ...
    for iLep in range(tree.naLeptons):
        ...
        if abs(tree.aLeptons_pdgId[iLep]) != 11: continue
        
        if tree.aLeptons_mcMatchId[iLep] != 23: continue

        if abs(tree.aLeptons_eta[iLep]) > 2.5: continue

        # Anythin past here is our denominator
        count['num_passed_aLepton_cuts'] += 1
        ...
        alep_iso.Fill(tree.aLeptons_relIso03[iLep])
        #alep_id.Fill(tree.aLeptons_eleMVArawSpring15NonTrig[iLep])
        alep_id.Fill(tree.aLeptons_eleMVAIdSpring15Trig[iLep])
        alep_id_noTrig.Fill(tree.aLeptons_eleMVAIdSpring15NonTrig[iLep])
        alep_eta.Fill(tree.aLeptons_eta[iLep])
        alep_eta_id.Fill(tree.aLeptons_eta[iLep], tree.aLeptons_eleMVAIdSpring15NonTrig[iLep])
